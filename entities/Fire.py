...
from traits.stand import StandTrait
...
class FireFlower(EntityBase):
    def __init__(self, screen, spriteColl, x, y, level, sound):
        super(FireFlower, self).__init__(y, x - 1, 1.25)
        self.spriteCollection = spriteColl
        self.animation = Animation(
            [
                self.spriteCollection.get("fire").image,
            ]
        )
        self.screen = screen
        self.leftrightTrait = StandTrait(self, level)
        self.type = "Mob"
