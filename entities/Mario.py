...
from traits.throw import throwFire
...
fireAnimation = Animation(
    [
        spriteCollection["mario_fire_run1"].image,
        spriteCollection["mario_fire_run2"].image,
        spriteCollection["mario_fire_run3"].image,
    ],
...
class Mario(EntityBase):
    ...
    def _onCollisionWithMob(self, mob, collisionState):
        if isinstance(mob, RedMushroom) and mob.alive:
        ...
        elif isinstance(mob, FireFlower) and mob.alive:
            self.powerup(2)
            self.killEntity(mob)
            self.sound.play_sfx(self.sound.powerup)
            ...
            if self.powerUpState == 0:
                ...
                x, y = self.rect.x, self.rect.y
                self.rect = pygame.Rect(x, y + 32, 32, 64)
                self.invincibilityFrames = 60
                self.sound.play_sfx(self.sound.pipe)
    ...
    def powerup(self, powerupID):
        if self.powerUpState < powerupID:
            if powerupID == 1:
                self.powerUpState = 1
                self.traits["goTrait"].updateAnimation(bigAnimation)
            ...
            if powerupID == 2:
                self.powerUpState = 2
                self.traits["goTrait"].updateAnimation(fireAnimation)
                self.rect = pygame.Rect(self.rect.x, self.rect.y - 32, 32, 64)
                self.invincibilityFrames = 20
